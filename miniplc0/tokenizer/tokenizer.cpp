		...
		while (true) {
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				if(!current_char.has_value()) { // 读到文件尾
                    ...
                    try {
                        return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, std::stoi(str), pos, currentPos()), std::optional<CompilationError>());
				...
				auto ch = current_char.value();
                if(miniplc0::isdigit(ch)) // 数字，存储
                    ss << ch;
                else if(miniplc0::isalpha(ch)) { //字母，存储并切换标识符状态
                    ss << ch;
